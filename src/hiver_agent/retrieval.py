...
class Index:
    ...
    def retrieve(self, query_vec: np.ndarray, k: int,
                 exclude_ids: set | None = None) -> list[dict]:
        q = query_vec / (np.linalg.norm(query_vec) + 1e-9)
        sims = self.matrix @ q
        exclude = exclude_ids or set()
        top = [i for i in np.argsort(-sims) if self.ids[i] not in exclude][:k]
        return [
    ...
    def retrieve_one(self, text: str, k: int, exclude_ids: set | None = None) -> list[dict]:
        from .llm import embed

        return self.retrieve(embed([text])[0], k, exclude_ids)
